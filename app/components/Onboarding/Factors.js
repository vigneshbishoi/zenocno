...
export default function Factors(props) {
  ...
  const { modalDisplay, setModalDisplay,onCheckBoxPressed ,noneCheckboxPressed,checkBoxFactorData,noneFactorSelected} = props;
  // const [noneFactorSelected, setNoneSelected] = useState(props.selectedId == "None" ? true : false);
  // const [selected, setSelected] = useState([]);
  // const [checkBoxFactorData, setCheckBoxData] = useState([]);
  ...
  //   const factorsWithSelected = factors && factors[0]?.data.map(item => { selected?.includes(item.id.toString()) ? item.selected = true : item.selected = false; return item });
  //   setCheckBoxData(factorsWithSelected);
  // }, [factors]);
  ...
  //   // let updatedData = isCheckedAlready(checkBoxFactorData)
  //   let updatedData = [...checkBoxFactorData];
  ...
  const getSelectedData = () => {
    if (noneFactorSelected) return "None"
    // Getting the names of all selected checkboxes
    let selectedData = checkBoxFactorData?.filter(data => data.selected).map(data => data.condition);
  ...
  const getSelectedId = () => {
    if (noneFactorSelected) return "None"
    // Getting the names of all selected checkboxes
    let selectedData = checkBoxFactorData?.filter(data => data.selected).map(data => data.id);

    // returning array seperated values as a string
    return selectedData?.join(",")
  }
  const closeModal = () => {
    const selectedData = getSelectedData(checkBoxFactorData);
    const selectedId = getSelectedId(checkBoxFactorData);
    props.updateFormData(selectedData, selectedId);
    setModalDisplay(false);
  }

  // const noneCheckboxPressed = () => {
  //   setCheckBoxData(isCheckedAlready(checkBoxFactorData))
  //   setNoneSelected(!noneFactorSelected);
  // }
  ...
  return (
    ...
    // >
      <View style={styles.container}>
        ...
        <CustomText style={styles.titleText}>{translate("ONBOARDING")["FACTORS_BEFORE"]}</CustomText> */}

        <CheckBox theme={theme} textContent={"None"} selected={noneFactorSelected} onPress={noneCheckboxPressed} />
        {/* List of checkboxes */}
        {/* <FlatList
          data={checkBoxFactorData}
          renderItem={renderItem}
          keyExtractor={item => item.id}
        /> */}
        {checkBoxFactorData?.map((item, index) => {
          return (<CheckBox key={item.id} id={item.id} theme={theme} textContent={item.condition} selected={item.selected} onPress={onCheckBoxPressed} />)
        })}
